<template>
  <teleport to="#modal-container">
    <transition name="modal-animation">
      <div class="fixed inset-0 z-10 bg-black-rgba flex justify-center"
           v-if="isOpenModal"
      >
        <div class="bg-floral-white w-[600px] mb-[12.5rem] mt-8 rounded-md" v-outside-click="close">
          <div class="flex items-center justify-between px-6 pt-6">
            <h1 class="font-body font-semibold text-2xl text-seal-brown">{{title}}</h1>
            <button
                @click="close">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M6.70925 5.29327C6.3183 4.9023 5.68436 4.90223 5.29332 5.29312C4.90228 5.68401 4.90222 6.31783 5.29318 6.7088L10.5854 12.0011L5.30107 17.2845C4.91007 17.6754 4.91007 18.3092 5.30107 18.7002C5.69207 19.0911 6.326 19.0911 6.717 18.7002L12.0011 13.417L17.2907 18.7067C17.6817 19.0977 18.3156 19.0978 18.7067 18.7069C19.0977 18.316 19.0978 17.6822 18.7068 17.2912L13.4171 12.0013L18.7046 6.71474C19.0956 6.32381 19.0956 5.68999 18.7046 5.29906C18.3136 4.90813 17.6796 4.90813 17.2886 5.29906L12.0013 10.5855L6.70925 5.29327Z"
                      fill="#3C1F1D"/>
              </svg>
            </button>
          </div>
          <slot></slot>
        </div>
      </div>
    </transition>
  </teleport>
</template>

<script>
import {outsideClick} from "@/directives/outsideClick";

export default {
  name: "ModalComponent",
  props: {
    isOpenModal: {
      type: Boolean,
      required: true
    },
    title: {
      type: String,
      default: '',
    }
  },
  directives: {
    outsideClick
  },
  setup(props, {emit}) {
    function close() {
      emit("close");
    }

    return {
      close
    }
  }
}
</script>
<style lang="scss">
.modal-animation-enter-active,
.modal-animation-leave-active {
  transition: all 0.5s ease;
}

.modal-animation-enter-from,
.modal-animation-leave-to {
  opacity: 0;
}
</style>
